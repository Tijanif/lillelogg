...
export async function GET(req: Request) {
    try {
        ...
        // --- Aggregation Logic for Diaper Counts ---
        const endDate = new Date();
        const startDate = getAnalyticsStartDate(days);

        const rawDiapers = await prisma.diaper.groupBy({
            by: ['startTime'],
            where: {
                babyId: babyId,
        ...
        console.error('Error fetching diaper analytics:', error);
